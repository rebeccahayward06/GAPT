...
class SensorPipeline:
    ...
    def infer(self):
        ...
        # motion = how much the signal VARIES across the window (flat at rest)
        ...
        if motion < IDLE_MOTION:
            if self.last_sent != "idle":
                self.osc.send_message(OSC_PATH, [self.side, "idle", 0.0, size, speed])
                print(f"[ {self.side}] idle (motion={motion:.2f})")
                self.last_sent = "idle"
            self.votes.clear()          # so stale gestures don't fire when motion resumes
            return
        ...
        print(f"[raw] {gesture:<18} {conf:.0%}  motion={motion:.2f}")
        ...
        self.osc.send_message(OSC_PATH, [self.side, label, conf, size, speed])
        self.last_sent = top
        print(f"[ {self.side}] {label:<24} conf={conf:.0%} size={size:.2f} speed={speed:.2f}")


async def run_sensor(side, address, osc):
    ...
    try:
        ...
        print(f"[{side}] CONNECTION ERROR: {e}")
...
async def main():
    ...
    await asyncio.gather(*[
        run_sensor(side, addr, osc) for side, addr in SENSORS.items()
    ])
